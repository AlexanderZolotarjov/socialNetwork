import React from 'react';
import s from './MyPosts.module.css';
import Post from './Post/Post';

const MyPosts = (props) => {
    let PostsElements = props.profiledata.PostsData.map((post) => {
        return (
            <Post key={post.id} likescount={post.likesCount} message={post.message} />
        )
    }) // Массив элементов разметки, созданный на основе массива данных для постов

    let newPostElement = React.createRef();
    let addPost = () => {
        props.dispatch({
            type: 'ADD-POST'
        });
    }

    let onPostChange = () => {
        props.dispatch({
            type: 'CHANGE-AREA-POST',
            value: newPostElement.current.value,
        })
    }
    return (
        <div className={`${s.content__posts} ${s.posts}`}>
            <div className={s.posts__title}>Posts</div>
            <div className={s.posts__new}>
                <textarea onChange={ onPostChange } ref={newPostElement} placeholder="Please enter your message!" value={props.profiledata.newPostText.text}></textarea>
                <button onClick={ addPost }>Add post</button>
            </div>
            <div className={s.posts__body}>
                {PostsElements}
            </div>
        </div>
    )
}

export default MyPosts;